...
from common.utils import model_join_list
...
class Comment(DBase):
    ...
    # 新增一条评论
    def insert_comment(self, articleid, content, ipaddr, tempname):
        now = time.strftime('%Y-%m-%d %H:%M:%S')
        comment = Comment(userid=session.get('userid'), articleid=articleid,
                          content=content, ipaddr=ipaddr, createtime=now, updatetime=now, tempname=tempname)
    ...
    # 根据用户编号和日期进行查询是否已经超过每天5条限制
    def check_limit_per_5_ip(self):
        start = time.strftime("%Y-%m-%d 00:00:00")  # 当天的起始时间
        end = time.strftime("%Y-%m-%d 23:59:59")  # 当天的结束时间
        result = dbsession.query(Comment).filter(Comment.ipaddr == request.remote_addr,
                                                 Comment.createtime.between(start, end)).all()
        if len(result) >= 2:
            return True  # 返回True表示今天已经不能再发表评论
        else:
    ...
    def insert_reply(self, articleid, commentid, content, ipaddr, tempname):
        now = time.strftime('%Y-%m-%d %H:%M:%S')
        comment = Comment(userid=session.get('userid'), articleid=articleid,
                          content=content, ipaddr=ipaddr, replyid=commentid,
                          createtime=now, updatetime=now, tempname=tempname)
        dbsession.add(comment)
        dbsession.commit()

    # 查询原始评论与对应的用户信息，带分页参数
    def find_comment_with_user(self, articleid, start, count):
        result = dbsession.query(Comment, User).join(User, User.userid ==
